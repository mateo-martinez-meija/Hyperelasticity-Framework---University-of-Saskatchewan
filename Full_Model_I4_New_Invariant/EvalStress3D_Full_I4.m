% Function EvalStress3D calculates the Cauchy Stress for each value of 
% strain that we have in the Mechanical Data. The variable "model" 
% lets us choose what type of Strain-Energy Density Function we want to
% use. Currently the only anisotropic model is the polynomic model ('Poly').

% strain = array of strain percentages from mechanical data
% c = coefficients for the model
% model = array with model for isotropic and anisotropic SEDF
% M_ = Structure tensor

function Stress = EvalStress3D_Full_I4(M_,strain,c,model)

n = length(strain);
Iso = model(1);
Aniso = model(2);
Stress = [];
switch Iso
    case 'MR' % Mooney-Rivlin with parallel strain data
        for i=1:n
            Stress = [Stress;...
                MRCauchyStress3D_Full_I4(M_,strain(i),c,Aniso)];
        end
    case 'MR_Perp' % Mooney-Rivlin with perpendicular strain data
        for i=1:n
            Stress = [Stress;...
                MRCauchyStress3D_Full_I4_Perp(M_,strain(i),c,Aniso)];
        end
end
end
