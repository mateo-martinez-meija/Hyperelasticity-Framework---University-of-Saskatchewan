...
% calculating the second Piola-Kirchhoff tensor numerically for a polynomic
...
% we push it forward by using the deformation gradient. 

% data = XRD data (angles and intensity)
% s = strain percentage
% c = coefficients

function CauchyStress = PolyCauchyStress3D_Full_I4(data,s,c)
...
for i=1:length(psi)-1
   ...
   S_ = S_ + PolySEDFder_I4(lambda,n,c4)*psi(i)*delta*transpose(n)*n;
...
sigma = (1/det(F_))*F_*S_*transpose(F_);
