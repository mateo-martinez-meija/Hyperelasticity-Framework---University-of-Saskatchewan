...
function CauchyStress = CauchyStress3D_Full_I4(data,s,c4,model)
...
% I1 = trace(C_); % Confirmation needed

% We include the isotropic component
% S_ = 2*(a*eye(3) + b*(I1*eye(3) - C_) + S_);
S_ = 2*S_;

% Find the Cauchy stress in the Lagrangian configuration
sigmaAux = (1/det(det(F_)))*F_*S_*transpose(F_);
p = sigmaAux(3,3);
